...
const questions = [{
    ...
    },
    {
        ...
        when: (answers) => !answers.blockchain && answers.method == 'profane' || answers.method == 'last time user was active' || answers.method == 'memos received',
        validate: (name) => /[\w---.]{3,}/.test(name) // If blockchain == false and memos received as choice
    },
    {
        type: 'input', // Make an input for the user
        name: 'wif',
        message: 'What is your memo key?',
        when: (answers) => !answers.blockchain && answers.method == 'memos received' // If blockchain == false and memos received as choice
    }, {
        type: 'confirm', // Ask a question for true or false
        name: 'send',
        ...
        when: (answers) => answers.method == 'transfers', // If choice is transfers
        validate: (amount) => {
            if (typeof parseFloat(amount) == 'number') // Make sure it is a valid number
                return true; // Return true
            else
                return 'Please enter a valid amount'; // If amount is not valid
        },
...
inquirer.prompt(questions).then(answers => {
    ...
    switch (answers.method) { // Switch statement for the user choice
        case 'transfers': // In case of `transfers`
            detector.transfer.detect((answers.send) ? answers.senders : null, answers.amount + ' ' + answers.coin,
                (answers.receive) ? answers.receivers : null, (!answers.isMemo) ? null : answers.memo, (transfer) => { // Start monitoring blockchain by user inputs
                    ...
                    if (memo === '') // If memo is empty
                        log('Transfer from ' + chalk.cyanBright(from) + ' to ' + chalk.cyan(to) + ' with an amount of ' + chalk.yellowBright(amount)) // Log it
                    else // If memo is not empty
                        log('Transfer from ' + chalk.cyanBright(from) + ' to ' + chalk.cyan(to) + ' with an amount of ' + chalk.yellowBright(amount) + ' memo: ' + chalk.magentaBright(memo)) // Log it
                });
            break;
        case 'profane': // In case of `profane`
            let username; // Variable for username
            if (answers.blockchain === true) // If blockchain is true
                username = null; // Scan in all blockchain
            else
                username = answers.username // Scan by username
            detector.profane.detect(username, (detected) => { // Start monitoring blockchain by user inputs
                let [word, author] = detected; // Convert object into local variables.
                log('A profane has been detected: ' + chalk.red(word) + ' Said: ' + chalk.cyanBright(author)); // Log it
            });
            break;
