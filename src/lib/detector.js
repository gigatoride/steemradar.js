...
module.exports = {
  ...
  transfer: {
    ...
    detect: (senders, min_amount, receivers, target_memo, callback) => {
      ...
      if (!typeof parseInt(min_amount) === 'number' && !/(SBD|STEEM|\|)/.test(min_amount))
        throw new Error('Target amount is not valid expected 0.000 STEEM|SBD.'); // Check if senders has a valid array.
      if (typeof callback === 'function') {
        this.release = steem.api.streamTransactions('head', (err, res) => {
          // Listing for blockchain transactions.
          if (err === null && res.operations[0][0] == 'transfer') {
            ...
            if (
              senders === null
                ? true
                : senders.includes(from) && receivers === null // Check senders
                  ? true
                  : receivers.includes(to) && target_memo === null // Check receivers
                    ? true
                    : memo.includes(target_memo)
            ) {
              // Check memo
              ...
              if (reAmount.test(amount) && parseFloat(target_amount) <= parseFloat(amount))
                // Check if transfer account is bigger than or equal min_amount
                callback(res); // Callback transaction object
            }
          } else if (err !== null) callback(err); // Callback error.
        });
      }
    },
    ...
    detect: (username, callback) => {
      ...
      if (typeof callback === 'function') {
        this.release = steem.api.streamTransactions('head', (err, res) => {
          ...
          if (err === null)
            if (res.operations[0][0] === 'comment') {
              // Check if error is null
              // Check type of transaction
              let op = res.operations[0][1]; // Operations object
              if (username === null || op.author === username) {
                // Check if username is not null
                const one_word = utils.filter.profaneWord(op.body);
                if (typeof one_word === 'string')
                  // Check if there is a bad word.
                  callback([one_word, op.author]); // Return bad word and author.
              }
            }
        });
      }
  ...
  activity: {
    ...
    detect: (usernames, callback) => {
      ...
      if (typeof callback === 'function') {
        ...
        const update = () => {
          // Keep updating any further activities by callback timestamp.
          if (!this.run) return; // If run is false stop updating.
          steem.api
            .getAccountsAsync(usernames) // Request account history
            .then(res => {
              res = res.map(obj => {
                // Detector filter activity object
                return {
                  username: obj.name, // Username
                  timestamp: obj.last_bandwidth_update // Get latest bandwidth update
                };
              });
              if (!last || !utils.isTsDup(last, res)) {
                last = res;
              ...
              Promise.delay(300)
                .then(update)
                .catch(callback); // Promise for updating callbacks and error handler.
            })
  ...
  memo: {
    ...
    detect: (memoKey, username, callback) => {
      if (!utils.isUsername(username) && username !== null)
        throw new Error('Username is not valid or exist.'); // Check username
      utils.isMemoKey(memoKey, username, res => {
        if (!res) throw new Error('Memo key is not valid.'); // Check username
      });
      if (typeof callback === 'function') {
        ...
        const update = () => {
          if (!this.run) return; // If run is false stop updating.
          steem.api
            .getAccountHistoryAsync(username, -1, 100)
            .then(res => {
              let transfer;
              ...
              if (latest_timestamp !== received[1].timestamp) {
                ...
                if (memo !== '')
                  if (memo.charAt(0) === '#' && memoKey !== null) {
                    // Check if private memo
                    let privateMemo = utils.isMemo(memoKey, memo);
                    transfer =
                      privateMemo !== false
                        ? [from, amount, privateMemo]
                        : [from, amount, 'Wrong Memo Key.'];
                  } else transfer = [from, amount, memo];
                // It is private memo in case of (no wif key)
                else transfer = [from, amount, memo]; // It is public memo
                callback(transfer);
              }
              Promise.delay(300)
                .then(update)
