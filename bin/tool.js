  ...
  switch (
    ...
    case 'profane': // In case of `profane`
      ...
      steemRadar.profane.detect(username, detected => {
        ...
        log(
          'A profane has been detected: ' + chalk.red(word) + ' Said: ' + chalk.cyanBright(author)
        ); // Log it
      });
      break;
      ...
      steemRadar.activity.detect([answers.username], ts => {
        ...
        log(
          chalk.cyanBright(answers.username) +
            ' last activity: ' +
            chalk.yellowBright(dateFormat(ts[0].timestamp, 'dddd, mmmm dS, yyyy, h:MM:ss TT'))
        ); // Log it
      });
      break;
